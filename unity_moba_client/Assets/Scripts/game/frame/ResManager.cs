...
public class ResManager : MonoBehaviour
{
    ...
    {
        get { return PathUtil.streamingAssetPath; }
    }

    public static string persistentDataPath
    {
        get { return PathUtil.persistentDataPath; }
    }

    public static string temporaryCachePath
    {
        get { return PathUtil.temporaryCachePath; }
    }
    ...
    public class Asset
    {
        ...
        public Sprite CreateSprite(Vector2 pivot)
        {
            if (texture != null)
            {
                return Sprite.Create(texture, new Rect(0, 0, texture.width, texture.height), pivot, 100, 0,
            ...
            else
            {
                ...
                if (asset.path != AssetPath.Resources)
                {
                    ...
                    if (data == null)
                    {
                        ...
                        data = ReadFileBytes(url + (priorityLoadEncrypted ? "" : ".txt"));
                    }

                    if (data != null && asset.type != AssetType.AudioClip)
                    {
                        if (debug)
                        {
                        ...
                        yield return null;
                        if (m_LoadingAssets.ContainsKey(asset.pathName + asset.url))
                            m_LoadingAssets.Remove(asset.pathName + asset.url);
                        if (asset.type == AssetType.Sprite || asset.type == AssetType.Texture2D)
                        {
                            ...
                            texture.LoadImage(data, asset.textureReadonly);
                        ...
                        if (asset.type == AssetType.Sprite)
                        {
                            ...
                            asset.sprite = Sprite.Create(asset.texture,
                                new Rect(0f, 0f, asset.texture.width, asset.texture.height), Vector2.one * 0.5f, 100, 1,
                                asset.meshType);
                        ...
                        else if (asset.type == AssetType.Texture2D)
                        {
                            ...
                            HandleLoadedAsset(asset, onLoaded);
                                 ...
                                 asset.url.LastIndexOf(".xml") == asset.url.Length - 4)
                        {
                            yield return StartCoroutine(LoadingSpritesByXML(System.Text.Encoding.UTF8.GetString(data),
                                asset, onLoaded, onLoadFailed));
                        }
                        else if (asset.type == AssetType.Text)
                        {
                            asset.text = System.Text.Encoding.UTF8.GetString(data);
                            HandleLoadedAsset(asset, onLoaded);
                        }
                    ...
                    {
                        url = GetPath(asset) + asset.url;
                        isReversed = priorityLoadEncrypted ? true : false;
                        realUrl = url + (priorityLoadEncrypted ? ".txt" : "");
                        if (asset.type == AssetType.Sprites)
                        {
                            realUrl = url;
                            isReversed = false;
                        ...
                        if (www.error == null || www.error.Length == 0)
                        {
                            if (debug)
                            {
                                print("ResManager>>>>> Load Success :" + url);
                            }
                            ...
                            if (asset.type == AssetType.Sprite)
                            {
                                asset.texture.wrapMode = TextureWrapMode.Clamp;
                                asset.sprite = Sprite.Create(asset.texture,
                                    new Rect(0f, 0f, asset.texture.width, asset.texture.height), Vector2.one * 0.5f,
                                    100, 1, asset.meshType);
                                asset.sprite.name = asset.texture.name;
                                HandleLoadedAsset(asset, onLoaded);
                            }
                            else if (asset.type == AssetType.Texture2D)
                                     ...
                                     asset.url.LastIndexOf(".xml") == asset.url.Length - 4)
                            {
                                yield return StartCoroutine(
                                    LoadingSpritesByXML(www.text, asset, onLoaded, onLoadFailed));
                            }
    ...
    IEnumerator LoadingSpritesByXML(string config, Asset asset, Action<Asset> onLoaded,
        Action<Asset> onLoadFailed = null)
    {
        string atlasPath = asset.url.Substring(0, asset.url.LastIndexOf(".xml")) + ".png";
        bool isReversed = priorityLoadEncrypted ? true : false;
        string url = GetPath(asset) + atlasPath;

        byte[] data = ReadFileBytes(url + (priorityLoadEncrypted ? ".txt" : ""));
        if (data == null)
        {
            isReversed = priorityLoadEncrypted ? false : true;
            data = ReadFileBytes(url + (priorityLoadEncrypted ? "" : ".txt"));
    ...
    void ReadSpritesByXML(string config, Asset asset)
    {
        ...
        foreach (XmlNode xn in nodeList)
        {
            ...
            Sprite s = Sprite.Create(asset.texture, new Rect(x, asset.texture.height - h - y, w, h), Vector2.one * 0.5f,
                100, 1, asset.meshType);
            s.name = frameName;
    ...
    /// <param name="useSearchPath">是否使用预设的search path.</param>
    /// 
    public void LoadTexture2DFromStreamingAsset(string url, Action<Texture2D> onLoaded, bool isAsyncInIOS = true,
        bool readOnly = true, Action<string> onLoadFailed = null, bool useSearchPath = true)
    {
    ...
    IEnumerator WWWSyncLoadTexture2D(string url, Action<Texture2D> onLoaded, bool readOnly = true,
