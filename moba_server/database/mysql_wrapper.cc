...
static void connect_work(uv_work_t* req)
{
	...
	MYSQL* ret = mysql_real_connect(pConn, r->ip, r->uname, r->upwd, r->db_name, r->port, NULL, 0);
	if (ret)
	{
...
static void query_work(uv_work_t* req)
{
	...
	struct mysql_context* mysql_r = (struct mysql_context*)(r->context);

	if(mysql_r->is_closed)
	{
		return;
	}
	uv_mutex_lock(&mysql_r->lock);

	MYSQL* pConn = (MYSQL*)mysql_r->pConn;
