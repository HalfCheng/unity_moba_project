...
lua_proto_man_init(lua_State* tolua_S) {
	...
	proto_type = (int)lua_tointeger(tolua_S, 1);
...
static int lua_raw_read_header(lua_State* tolua_S)
{
	...
	raw = (struct raw_cmd*)tolua_touserdata(tolua_S, 1, NULL);
	if (NULL == raw)
	{
		goto lua_failed;
	}
...
lua_failed:
	log_error("raw_read_header error!!!");
	return 0;
}
...
static int lua_raw_set_utag(lua_State* tolua_S)
{
	...
	struct raw_cmd* raw;
	unsigned int utag;
	unsigned char* utag_ptr;
	if (argc != 2)
	{
		log_error("argc error!!");
		goto lua_failed;
	}
